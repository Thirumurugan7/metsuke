...
function updateStatusLine(update: (UpdateState & { enabled: boolean }) | null): string {
  ...
  switch (update.status) {
    ...
    case 'ready':
      return `Version ${update.version} is ready. Installing restarts the editor and ends every session.`
    case 'error':
...
/**
 * One dialog, four sections (A7) — replaces what used to be a notifications-only
 * modal. `settingsOpen`/`setSettingsOpen` are unchanged from before; only what they
 * gate got bigger.
 */
export function Settings(): JSX.Element | null {
  ...
  // Fresh tab every time it opens, rather than remembering wherever it was left last.
  useEffect(() => {
    if (settingsOpen) setTab('appearance')
  }, [settingsOpen])
  ...
  return (
    <Modal variant="dialog" label="Settings" onClose={() => setSettingsOpen(false)} className="settings-modal">
      ...
      <div className="settings-body">
        ...
        {tab === 'notifications' && (
          <>
            ...
            {!s ? (
              <p className="settings-note">Loading notification settings…</p>
            ) : (
              <>
                <section>
                ...
                <section>
                  ...
                  <label className="settings-row">
                    <input
                      ...
                      onChange={(e) =>
                        void updateNotifySettings({ sound: { ...s.sound, enabled: e.target.checked } })
                      }
                    />
                    <span>
                      Play a sound
        ...
        {tab === 'project' && (
          <section>
            ...
            <label className="settings-row">
              ...
              <span>
                Check the project when I open a folder
                <small>
                  Starts a Claude session that reads the codebase and reports back, every time you
                  open a folder here.
                </small>
              </span>
            </label>
          ...
          <section>
            ...
            {!telemetry?.configured ? (
              <p className="settings-note">
                No usage-reporting endpoint is compiled into this build, so there is nothing to send
                anywhere.
              </p>
            ) : (
              <>
